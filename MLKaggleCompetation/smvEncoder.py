...
# data = fullData.drop(['Id', 'NextId', 'Position'],axis = 1)
#data = fullData.drop(['Id', 'Prediction'],axis = 1)
# data = fullData
# nextId = pd.get_dummies(fullData['NextId'])
position = pd.get_dummies(fullData['Position'])
data = pd.concat([position,data],axis = 1)
# data = pd.concat([nextId,position,data],axis = 1)
...
def svm_fit(i,trainData, testData, trainTarget, testTarget):
    ...
    gammaList = pd.read_csv('EncoderparaList.csv')
    ...
    f= open("encoderSVMParaRunResult.txt","a+")
    f.write(outString)
    f.close()
    print(outString)
#    return accuracy
